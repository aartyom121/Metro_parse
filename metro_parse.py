...
# last_page = requests.get('https://online.metro-cc.ru/category/zamorozhennye-produkty/morozhenoe?in_stock=1', headers=headers)
# link = f"https://online.metro-cc.ru/category/zamorozhennye-produkty/morozhenoe?in_stock={params['in_stock']}?page={params['page']}"

metro_dn = 'https://online.metro-cc.ru'


def get_response(lk, pm, cs, hs):
    rs = requests.get(lk, params=pm, cookies=cs, headers=hs).text
    return rs


def get_data(sp, pm, cs, hs, results):
...
def get_brand(lk, pm, cs, hs):
    rs = get_response(lk, pm, cs, hs)
...
def get_last_page():
    ...
    res = get_response(link, params, cookies, headers)
...
def main():
    ...
    for i in range(int(last_page)):
        link = f'https://online.metro-cc.ru/category/zamorozhennye-produkty/morozhenoe'
        response = get_response(link, params, cookies, headers)
        print(link)
        soup = BeautifulSoup(response, 'lxml')
        get_data(soup, params, cookies, headers, results)
        page += 1
